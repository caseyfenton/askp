...
with open('src/askp/utils.py', 'r', encoding='utf-8') as f:
    content = f.read()

# Check for basic syntax before attempting fix
import ast
try:
    ast.parse(content)
    print("No syntax errors found! (Unexpected)")
except SyntaxError as e:
    print(f"Syntax error confirmed: {e}")

# Create a simplified version that removes all multi-line strings and replaces them with simple strings
# This can help identify where the issue is
import re
simplified = re.sub(r'"""[\s\S]*?"""', '"""DOCSTRING"""', content)
simplified = re.sub(r"'''[\s\S]*?'''", "'''DOCSTRING'''", simplified)

# Try to parse the simplified version
try:
    ast.parse(simplified)
    print("Simplified version has no syntax errors")
except SyntaxError as e:
    print(f"Syntax error in simplified version: {e}")

# Try a hard reset approach - create a brand new utils.py with minimal correct functionality
with open('src/askp/utils.py', 'w', encoding='utf-8') as f:
...
print("Created a clean utils.py file with fixed syntax")
